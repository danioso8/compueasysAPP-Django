...
(function () {
  ...
  function escapeHtml(s) { return (s === null || s === undefined) ? '' : String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
    ...
    try {
      ...
      const html = `
        <div id="${id}" class="toast align-items-center text-bg-${variant} border-0 mb-2" role="alert" aria-live="assertive" aria-atomic="true">
          <div class="d-flex">
            <div class="toast-body"><strong class="d-block">${escapeHtml(title)}</strong><div>${escapeHtml(message)}</div></div>
  ...
  function showConfirm(opts) {
    if (window.Swal && typeof Swal.fire === 'function') {
      Swal.fire({
        ...
        reverseButtons: true,
      }).then((result) => { if (result.isConfirmed && typeof opts.onConfirm === 'function') opts.onConfirm(); });
    } else {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // preview main image
    const previewMainImg = safeEl('preview-main-img') || safeEl('preview-main-img-edit');

    // gallery input and container (ids used in template)
    const galInput = safeEl('producto_galeria') || null;
    const galleryContainer = safeEl('preview-gallery') || safeEl('preview-gallery-edit') || null;
    ...
    function renderGalleryPreviews() {
      ...
      Array.from(galleryContainer.querySelectorAll('.gallery-item[data-new-index]')).forEach(n => n.remove());
    ...
    if (galInput && galleryContainer) {
      ...
      galInput.addEventListener('change', function (e) {
        ...
        files.forEach(f => {
          const exists = galleryFiles.some(g => g.name === f.name && g.size === f.size && g.lastModified === f.lastModified);
    ...
    function existingGalleryRemoveHandler(e) {
      ...
      if (formEl) {
        ...
        const existing = formEl.querySelectorAll('input[name="galeria_existing[]"]');
        existing.forEach(inp => { if (inp.value === id) inp.remove(); });
      }
      // remove DOM element
      this.closest('.gallery-item')?.remove();
    }
    ...
    function initVariantRemoveHandlers() {
      ...
      variantesContainer.querySelectorAll('.variant-remove-btn').forEach(btn => {
        btn.removeEventListener('click', variantRemoveHandler);
        btn.addEventListener('click', variantRemoveHandler);
      });
      // new row remove buttons
      variantesContainer.querySelectorAll('.variant-remove-btn-new').forEach(btn => {
    ...
    function fillProductFormFromData(data) {
      ...
      const setIf = (id, val) => { const el = safeEl(id); if (el) el.value = (val === null || val === undefined) ? '' : val; };
      ...
      setIf('producto_price_buy_edit', (data.price_buy !== undefined) ? data.price_buy : '');
      ...
      setIf('producto_descuento_edit', (data.descuento !== undefined) ? data.descuento : '');
      ...
      if (theForm && data.id) {
        ...
        const baseAction = theForm.dataset.createAction || window.location.pathname;
        ...
        try {
          ...
          const submitBtn = safeEl('form-submit-btn') || safeEl('form-submit-btn-edit') || null;
      ...
      if (mainImg) {
        if (data.imagen) { mainImg.src = data.imagen; mainImg.style.display = 'block'; }
      ...
      if (galleryContainer) {
        ...
        galleryArr.forEach(function (g) {
          ...
          if (typeof g === 'string') { img.src = g; div.dataset.galleryId = ''; }
          else { img.src = g.url || g.imagen || g.galeria || ''; div.dataset.galleryId = g.id || ''; if (g.id) div.dataset.galleryId = g.id; }
          div.appendChild(img);
          // remove btn
          const btn = document.createElement('button');
          btn.type = 'button';
          ...
          btn.style.top = '4px';
          btn.style.right = '4px';
          if (div.dataset.galleryId) btn.dataset.galleryId = div.dataset.galleryId;
          div.appendChild(btn);
          galleryContainer.appendChild(div);
        });
      ...
      if (variantesContainer) {
        ...
        const vs = Array.isArray(data.variants) ? data.variants : (data.variants_list || []);
        if (vs.length) {
          vs.forEach(function (v) {
            ...
            rowDiv.innerHTML =
              ...
              `<input type="text" name="variante_nombre[]" class="form-control" value="${escapeHtml(v.nombre)}" placeholder="Nombre variante">` +
              `<input type="number" name="variante_precio[]" class="form-control" value="${escapeHtml(v.precio)}" step="0.01" placeholder="Precio">` +
              `<input type="number" name="variante_stock[]" class="form-control" value="${escapeHtml(v.stock)}" placeholder="Stock">` +
              `<input type="text" name="variante_color[]" class="form-control" value="${escapeHtml(v.color)}" placeholder="Color">` +
              `<input type="text" name="variante_talla[]" class="form-control" value="${escapeHtml(v.talla)}" placeholder="Talla">` +
              `<input type="file" name="variante_imagen[]" class="form-control">`;
    ...
    rows.forEach(function (row) {
      ...
      row.addEventListener('click', function (e) {
        ...
        fetch(apiUrl, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
          ...
          .then(data => {
            if (!data || data.error) { window.location.href = `${window.location.pathname}?view=productos&crear=1&editar=${productId}`; return; }
            ...
            try { history.replaceState({}, '', `${window.location.pathname}?view=productos&crear=1&editar=${productId}`); } catch (e) {}
          })
          .catch(err => {
    ...
    /* ---------- Clear/New button ---------- */
    const clearBtn = safeEl('form-clear-btn') || safeEl('form-clear-btn-create') || safeEl('form-clear-btn-edit');
    if (clearBtn && form) {
      clearBtn.addEventListener('click', function () {
        form.reset();
        // clear product_id hidden(s)
        const hid1 = safeEl('producto_id'); if (hid1) hid1.value = '';
        const hid2 = safeEl('producto_id_edit'); if (hid2) hid2.value = '';
        // clear previews and state
        if (previewMainImg) { previewMainImg.src = ''; previewMainImg.style.display = 'none'; }
        if (galleryContainer) { galleryContainer.innerHTML = ''; }
        if (variantesContainer) { variantesContainer.innerHTML = ''; }
        galleryFiles = [];
        // reset form action to create-action (if set)
        if (form.dataset && form.dataset.createAction) form.action = form.dataset.createAction;
        // reset submit label
        const submitBtn = safeEl('form-submit-btn') || safeEl('form-submit-btn-create');
    ...
    try {
      ...
      if (editarId && form) {
        ...
        fetch(apiUrl, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
          ...
          .then(data => { if (data && !data.error) fillProductFormFromData(data); })
        ...
        onConfirm: function () {
          ...
          .then(data => {
            if (data && data.success) {
              ...
              showToast('Eliminado', 'Producto eliminado correctamente', 'success', 2000);
            } else {
              const err = (data && data.error) ? data.error : 'No se pudo eliminar el producto';
              showToast('Error', err, 'danger', 4000);
            }
          })
          .catch(err => {
    ...
    document.querySelectorAll('.delete-product-btn, .btn-eliminar-producto').forEach(btn => {
    ...
    document.addEventListener('click', function (e) {
      const del = e.target.closest && e.target.closest('.delete-product-btn, .btn-eliminar-producto');
      if (!del) return;
      e.preventDefault();
      const pid = del.dataset.productId || del.getAttribute('data-product-id');
